...
class AdminDashboard {
    ...
    loadCertificates() {
        const certificates = this.store.getData('certificates');
            ...
            <div class="item-card">
                <div class="item-info">
                    ...
                    <small style="color: var(--color-text-tertiary); display: block; margin-top: 0.5rem;">
                        ${cert.fromDate ? new Date(cert.fromDate).toLocaleDateString() : ''} - ${cert.toDate ? new Date(cert.toDate).toLocaleDateString() : ''}
                    </small>
                </div>
                <div class="item-actions">
                    <button class="btn-icon" onclick="window.adminDashboard.showCertificateModal(${cert.id})">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn-icon delete" onclick="if(confirm('Delete this certificate?')) { window.adminDashboard.store.deleteItem('certificates', ${cert.id}); window.adminDashboard.loadCertificates(); window.adminDashboard.loadDashboard(); alert('Certificate deleted!'); }">
                        <i class="fas fa-trash"></i>
                    </button>
                </div>
            </div>
            ...
            <form id="itemForm" class="admin-form">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="url" name="github" value="${project.github || ''}" placeholder="https://github.com/...">
                    </div>
                    <div class="form-group">
                        <label>Demo Link (optional)</label>
                        <input type="url" name="demo" value="${project.demo || ''}" placeholder="https://demo.com/...">
                    </div>
                </div>
            ...
            <form id="itemForm" class="admin-form">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="date" name="fromDate" value="${cert.fromDate || ''}" required>
                    </div>
                    <div class="form-group">
                        <label>To Date</label>
                        <input type="date" name="toDate" value="${cert.toDate || ''}" required>
                    </div>
                </div>
                <div class="form-group">
                    <label>Certificate PDF</label>
                    <input type="file" name="pdf" accept=".pdf,application/pdf" id="pdfInput">
                    <small style="color: var(--color-text-tertiary);">Upload certificate PDF file</small>
                </div>
                <div class="form-group">
                    <label>Verification Link (optional)</label>
                    <input type="url" name="link" value="${cert.link || ''}" placeholder="https://verify.example.com/...">
                </div>
                <button type="submit" class="btn btn-primary">
